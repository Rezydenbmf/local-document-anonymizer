...
adds safe OCR metadata to per-file reports and batch summaries.
...
Batch summary reports may contain safe filenames and OCR status metadata, but
they must not contain full paths, source document text, raw OCR text, private
...
documents and does not receive original source text, raw OCR text, original
private dictionary aliases or terms, text snippets, raw exception text, or
replacement maps.
...
text is not written to reports or batch summaries.
