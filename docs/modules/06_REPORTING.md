...
local LLM review metadata to per-file reports and batch summaries.
...
status metadata, and LLM review metadata, but they must not contain full
paths, source document text, raw OCR text, detected entity text, raw LLM
prompts, raw LLM responses, document snippets, private dictionary terms,
dictionary aliases, raw exception messages, or replacement maps.
...
metadata, safe NER metadata, safe LLM review metadata, audit metadata including
risk level, file-type metadata, safe filenames, and controlled batch error
descriptions. It does not read source documents and does not receive original
source text, raw OCR text, detected entity text, raw LLM prompts, raw LLM
responses, original private dictionary aliases or terms, text snippets, raw
