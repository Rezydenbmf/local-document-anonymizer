...
safe NER metadata to per-file reports and batch summaries.
...
Batch summary reports may contain safe filenames, OCR status metadata, and NER
status metadata, but they must not contain full paths, source document text,
raw OCR text, detected entity text, private dictionary terms, dictionary
aliases, raw exception messages, or replacement maps.
...
metadata, safe NER metadata, audit metadata including risk level, file-type
metadata, safe filenames, and controlled batch error descriptions. It does not
read source documents and does not receive original source text, raw OCR text,
detected entity text, original private dictionary aliases or terms, text
snippets, raw exception text, or replacement maps.
