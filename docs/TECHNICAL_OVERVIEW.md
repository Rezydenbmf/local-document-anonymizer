...
category counters, output path, report path, and the manual review warning.
...
`report.py` builds and saves safe TXT reports without original sensitive source values. It receives only status, input type, output type, category counters, and optional category ordering. It does not receive source document text, full input paths, source filenames, or replacement maps.
...
Stage 6 does not create a replacement map and does not include source values in
returned counters, report files, or GUI status. The project
