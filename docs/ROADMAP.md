...
| Stage 13 | Manual review workflow | Detect generated `_ANON` outputs in an output folder, pair matching `_RAPORT` reports when present, let the user assign manual `approved`, `needs_review`, or `rejected` statuses, and save safe review status and summary metadata. | Current | Status tracking only; no preview, editor, automatic approval, file moving, OCR, AI, APIs, local LLM, cloud service, or database. |
